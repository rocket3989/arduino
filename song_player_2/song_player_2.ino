...
void loop(void)
{
	...
	if (player_state == NEXT_SONG)
	{
		...
		switch (song_selected)
		{
			case 0:
				...
				song_playing = pgm_read_byte_near(song0 + k));
				break;
			case 1:
				Serial.println("playing song 2");
				song_playing = pgm_read_byte_near(song1 + k));
				break;
			case 2:
				Serial.println("playing song 3");
				song_playing = pgm_read_byte_near(song2 + k));
				break;
			case 3:
				Serial.println("playing song 4");
				song_playing = pgm_read_byte_near(song3 + k));
				break;
			case 4:
				Serial.println("playing song 5");
				song_playing = pgm_read_byte_near(song4 + k));
				break;

			case 5:
				Serial.println("playing song 6");
				song_playing = pgm_read_byte_near(song5 + k);
				break;
				/*
				case 6:
	...
	if (player_state == PLAY)
	{
		...
		if(note++ == song_selected.sizeOf()){//how do you get len of char array/?
			note = 0;
			player_state = 0;
	}
